...
#             qrels_file = os.path.join(script_dir, "dev1.tsv")
#         if collection_name == "mt-rag-fiqa-beir-elser-512-100-20240501":
#             qrels_file = os.path.join(script_dir, "dev2.tsv")
#         if collection_name == "mt-rag-ibmcloud-elser-512-100-20240502":
#             qrels_file = os.path.join(script_dir, "dev3.tsv")
#         if collection_name == "custom-eval":
#             qrels_file = os.path.join(script_dir, "my_eval_qrels.tsv")
...
import argparse
import csv, json
import pandas as pd  # Needed for enrich_json_retrieval
from judge_utils import *
...
def compute_results(results, qrels):
    k_values = [1, 3, 5]
    if len(results) == 0:
        ndcg = _map = recall = precision = {i: '-' for i in k_values}
    else:
        scores_per_query_id, ndcg, _map, recall, precision = evaluate(qrels, results, k_values)

    scores_global = {
        "nDCG": list(ndcg.values()),
...
def prepare_results_dict(input_file):
    ...
    with open(input_file, 'r') as f:
        for line in f:
            item = json.loads(line)
            query_id = item["task_id"]
            doc_scores = {ctx["document_id"]: ctx["score"] for ctx in item.get("contexts", [])}
            results[query_id] = doc_scores
            collection_results[query_id] = item["Collection"]
    return results, collection_results
...
# --- Main evaluation function that can be called from code ---
def run_retrieval_evaluation(input_file: str, output_file: str):
    retrieval_predictions, collection_results = prepare_results_dict(input_file)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    for collection_name in collections:
        if collection_name == "mt-rag-govt-elser-512-100-20240611":
            qrels_file = os.path.join(script_dir, "dev1.tsv")
        elif collection_name == "mt-rag-fiqa-beir-elser-512-100-20240501":
            qrels_file = os.path.join(script_dir, "dev2.tsv")
        elif collection_name == "mt-rag-ibmcloud-elser-512-100-20240502":
            qrels_file = os.path.join(script_dir, "dev3.tsv")
        elif collection_name == "custom-eval":
            qrels_file = os.path.join(script_dir, "my_eval_qrels.tsv")
        else:
            continue
    ...
    n = len(scores_global_lst[0]['Recall'])
    total_count = sum(d['count'] for d in scores_global_lst)
    weighted_avg_recall = [sum(d['Recall'][i] * d['count'] for d in scores_global_lst)/total_count for i in range(n)]
    weighted_avg_ndcg = [sum(d['nDCG'][i] * d['count'] for d in scores_global_lst)/total_count for i in range(n)]
    ...
    return {
        "global_scores": scores_global_lst,
        "weighted_avg": {
            "nDCG": weighted_avg_ndcg,
            "Recall": weighted_avg_recall
        },
        "scores_per_query": global_scores_per_query_id
    }
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    run_retrieval_evaluation(args.input_file, args.output_file)
